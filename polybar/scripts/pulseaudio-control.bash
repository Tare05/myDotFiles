...
# Maps PulseAudio Port names to human-readable names
...
function switchPort() {
    ...
    getCurPort
    if [ "$curPort" = "analog-output-lineout" ]; then
...
function output() {
    ...
    if [ "$isMuted" = "yes" ]; then
        echo "${MUTED_COLOR}${MUTED_ICON}   ${GODS_FAV_COLOR}${SINK_ICON}${END_COLOR}${portNickname}${END_COLOR}"
    else
        echo "${GODS_FAV_COLOR}${volIcon}${END_COLOR}${curVol}%   ${GODS_FAV_COLOR}${SINK_ICON}${END_COLOR}${portNickname}"
    fi
}
